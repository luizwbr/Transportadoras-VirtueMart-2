 ...
 *
 */
class plgVmShipmentTransportadoras extends vmPSPlugin {
	...
	public function getVmPluginCreateTableSQL () {
		...
		return $this->createTableSQL ('Shipment Transportadoras Table');
	}
	...
	function getOrderShipmentHtml ($virtuemart_order_id) {
		...
		$html .= $this->getHtmlRowBE ('TRANSPORTADORAS_SHIPPING_NAME', $shipinfo->shipment_name);
		$html .= $this->getHtmlRowBE ('TRANSPORTADORAS_WEIGHT', $shipinfo->order_weight . ' ' . ShopFunctions::renderWeightUnit ($shipinfo->shipment_weight_unit));
		$html .= $this->getHtmlRowBE ('TRANSPORTADORAS_COST', $currency->priceDisplay ($shipinfo->shipment_cost));
		$html .= $this->getHtmlRowBE ('TRANSPORTADORAS_PACKAGE_FEE', $currency->priceDisplay ($shipinfo->shipment_package_fee));
		$html .= $this->getHtmlRowBE ('TRANSPORTADORAS_TAX', $taxDisplay);
		$html .= '</table>' . "\n";
	...
	function getCosts (VirtueMartCart $cart, $method, $cart_prices) {
		...
		if ($method->free_shipment && $cart_prices['salesPrice'] >= $method->free_shipment) {
			...
			// activate cost per un
			if ($method->type_cost == 1) {
				$cost_per_un = $method->cost_per_un;
				$weight = $this->getOrderWeight ($cart, $method->weight_unit);
				$cost_value = $cost_per_un * $weight;
				return $cost_value + $method->package_fee;
			} else {
	...
	protected function checkConditions ($cart, $method, $cart_prices) {
		...
		if (!is_array ($address)) {
			...
			if ($method->zip_start or $method->zip_stop or empty($countries)) {
				...
				$redirectMsg = JText::_ ('VMSHIPMENT_TRANSPORTADORAS_ADDRESS_FIRST');
				//vmWarn($redirectMsg);
		...
		if (!empty($this->_psType) and !$this->selectedThis ($this->_psType, $name, $id)) {
			...
			foreach($toConvert as $field){
				...
				if (preg_match ("/[^0-9]/", $data[$field])) {
					vmWarn( JText::sprintf('VMSHIPMENT_TRANSPORTADORAS_NUMERIC', JText::_('VMSHIPMENT_TRANSPORTADORAS_'.$field) ) );
				}
			}*/
			//Reasonable tests:
			if(!empty($data['zip_start']) and !empty($data['zip_stop']) and (int)$data['zip_start']>=(int)$data['zip_stop']){
				vmWarn('VMSHIPMENT_TRANSPORTADORAS_ZIP_CONDITION_WRONG');
			}
			if(!empty($data['weight_start']) and !empty($data['weight_stop']) and (float)$data['weight_start']>=(float)$data['weight_stop']){
				vmWarn('VMSHIPMENT_TRANSPORTADORAS_WEIGHT_CONDITION_WRONG');
			}
